...
from orm_core.models import Restaurant, Rating, Sale
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        user = User.objects.filter(username='manojpun')
        if not user.exists():
            user = User.objects.create_superuser(username='manojpun', password='password')
        else:
            user = user.first()
        ...
        restaurants = [
            ...
            {'name': 'Bombay Bustle', 'date_opened': timezone.now() - timezone.timedelta(days=44), 'restaurant_type': Restaurant.TypeChoices.NEPALESE, 'latitude': 51.509865, 'longitude':  -0.118092},  # Changed to NEPALESE
            {'name': 'McDonalds', 'date_opened': timezone.now() - timezone.timedelta(days=51), 'restaurant_type': Restaurant.TypeChoices.OTHER, 'latitude': 53.483959, 'longitude':  -2.244644},  # Changed FASTFOOD to OTHER
            {'name': 'Taco Bell', 'date_opened': timezone.now() - timezone.timedelta(days=12), 'restaurant_type': Restaurant.TypeChoices.OTHER, 'latitude': 53.413959, 'longitude':  -2.254644},  # Changed FASTFOOD to OTHER
            ...
            {'name': 'Nepalese 2', 'date_opened': timezone.now() - timezone.timedelta(days=46), 'restaurant_type': Restaurant.TypeChoices.NEPALESE, 'latitude': 53.350140, 'longitude':  -6.266155},  # Changed Indian to Nepalese
            {'name': 'Mexican 1', 'date_opened': timezone.now() - timezone.timedelta(days=52), 'restaurant_type': Restaurant.TypeChoices.OTHER, 'latitude': 51.481583, 'longitude':  -3.179090},  # Changed MEXICAN to OTHER
            {'name': 'Mexican 2', 'date_opened': timezone.now() - timezone.timedelta(days=50), 'restaurant_type': Restaurant.TypeChoices.OTHER, 'latitude': 55.847258, 'longitude':  -4.440114},  # Changed MEXICAN to OTHER
